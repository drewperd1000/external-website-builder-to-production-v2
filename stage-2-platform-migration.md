...
| Cross-surface naming inventory | Grep project for any name that was renamed during migration; cross-check against external surfaces (Railway env vars, PostHog event names, GSC routes, vendor product IDs) | Per Principle 3 in [`_internal/reference-operating-principles.md`](_internal/reference-operating-principles.md) — every shared name updated lock-step in the same session, no surface left referring to the old name |
