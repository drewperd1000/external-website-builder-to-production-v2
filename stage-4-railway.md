...
Railway re-uses the deployment record in place across rebuilds — `id` and `createdAt` do NOT change between deploys. Only `meta.commitHash` reliably changes. See [`_internal/reference-railway-automation.md`](_internal/reference-railway-automation.md) for the full gotcha detail. The correct polling pattern:
